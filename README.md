...
[download v1.1.0](https://github.com/aldikosh23/Foldglass/releases/tag/v1.1.0) · [русская инструкция](docs/README.ru.md) · [build from source](#build-from-source) · [report an issue](https://github.com/aldikosh23/Foldglass/issues)
...
3. click **«дать доступ к экрану»** and enable foldglass in the screen recording section of privacy & security. accept an app restart if macos requests it.
4. open the lid past **92°**, then slowly lower it below **90°**. the effect should follow the lid and clear when it opens again.

the interface is currently in russian. controls are translated below; a [detailed russian guide](docs/README.ru.md) covers setup and troubleshooting.
...
| --- | --- |
| «проиграть» | plays a six-second animation inside the preview |
| «демо на экране» | plays the effect on the built-in display for six seconds |
| «обновить снимок» | replaces the preview image with a fresh desktop snapshot |
| «начало» | sets the angle below which the effect starts |
| «матовое стекло» | controls blur strength |
| «затемнение» | controls how quickly the screen fades |
| «растяжение» | controls perspective compensation around the hinge |
| «вернуть исходный эффект» | restores the effect settings |
...
**launch at login:** turn on **«запускать при входе в macos»** in settings. subsequent login launches stay in the background with a menu bar icon and no settings window. startup is opt-in and uses `SMAppService`. if macos requests approval, click **«разрешить в настройках»** and allow foldglass. disable startup using the same toggle or **system settings > general > login items & extensions**. keep the installed app in its original location while this is enabled.

**pause or stop:** «приостановить эффект» pauses the current session. «завершить foldglass» exits the app. quitting does not disable launch at login.

## privacy and limits

foldglass takes **one screenshot** of the built-in display when an effect starts. the image and its gpu textures stay in memory; the app does not save screenshots to disk. «обновить снимок» also captures a single image for the preview and keeps it in memory until replaced or the app quits.
...
- perceived perspective depends on your viewing position. tune «растяжение» to suit it.
