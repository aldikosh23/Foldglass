...
swiftc -swift-version 5 -O "$project_dir/Sources/LoginItem.swift" \
  ...
  -framework AppKit -o "$test_dir/launch-context"
"$test_dir/launch-context"
swiftc -swift-version 5 -O "$project_dir/Sources/Localization.swift" \
  "$project_dir/Tests/LocalizationTests.swift" -o "$test_dir/localization"
"$test_dir/localization" "$project_dir"
